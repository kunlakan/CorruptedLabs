...
public class CorruptedFunctionalAPI extends Game
{
    ...
    public GridElement.ColorEnum getColor(String color)
    {
        ...
        else if(color.equalsIgnoreCase("arrow"))
        {
            return GridElement.ColorEnum.BLUE;   
        }
        else if(color.equalsIgnoreCase("flower"))
        {
            return GridElement.ColorEnum.GREEN;   
        }
        else if(color.equalsIgnoreCase("crab"))
        {
...
public     int flowerX = 4;
public     int flowerY = 4;
    public class Enemy {
        int delay = 0;
        final int DropRate = 10;
        public void Enemy() {
            newPos();
        }
        private void newPos() {
        ...
        public void update() {
            ...
            if (delay > DropRate) {
                delay = 0;
                markTileForDelete(arrowX, arrowY);
                if (arrowY > 0) {
                   arrowY--;
                   drawTile(arrowX, arrowY, "arrow");
                } else {
                    newPos();
                }
            }
    ...
    public void drawArrow() {
        ...
        // Hide these
    public void fallCrab() {
        boolean shouldFall = (delayCount > FallDelay) && (crabY > 0);
        delayCount++;
        if (shouldFall) {
            markTileForDelete(crabX, crabY);
            delayCount = 0;
            crabY--;
            drawTile(crabX, crabY, "crab");
        }
    }   
        
    public boolean crabNotOnPlatform(int x0, int x1, int y) {
        return ((crabY != (y+1)) || (crabX < x0) || (crabX > x1));
    }
